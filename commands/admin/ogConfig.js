...
/**
 * DEPRECATED: Legacy OG Config command
 * Redirects to /verification admin og-* commands
 */
module.exports = {
  data: new SlashCommandBuilder()
    .setName('og-config')
    .setDescription('⚠️ DEPRECATED - Use /verification admin og-* instead')
  ...
  async execute(interaction) {
    ...
    const deprecationEmbed = new EmbedBuilder()
      ...
      .setDescription(
        '`/og-config` is deprecated and will be removed in a future update.\n\n' +
        '**Please use the new location:**\n' +
        '• `/verification admin og-view` - View OG config\n' +
        '• `/verification admin og-enable` - Enable/disable OG system\n' +
        '• `/verification admin og-role` - Set OG role\n' +
        '• `/verification admin og-limit` - Set OG member limit\n' +
        '• `/verification admin og-sync` - Sync OG roles\n\n' +
        '*Executing your command with the old handler...*'
      )
      .setTimestamp();

    await interaction.editReply({ embeds: [deprecationEmbed] });

    // Wait a moment so user sees the deprecation notice
    ...
    try {
      ...
      await interaction.editReply({ 
        content: '❌ An error occurred while processing your request.\n\n**Tip:** Try using the new `/verification admin og-*` commands instead.' 
      });
    }
  },
};
...
async function handleView(interaction) {
  ...
  embed.setFooter({ text: 'NEW: Use /verification admin og-sync to apply changes' });

  await interaction.editReply({ embeds: [embed] });
  logger.log(`Admin ${interaction.user.tag} viewed OG config (legacy command)`);
}
...
async function handleEnable(interaction) {
  ...
  if (result.success && enabled) {
    embed.setFooter({ text: 'NEW: Use /verification admin og-sync to apply the role!' });
  }

  await interaction.editReply({ embeds: [embed] });
  logger.log(`Admin ${interaction.user.tag} ${enabled ? 'enabled' : 'disabled'} OG role (legacy command)`);
}
...
async function handleSetRole(interaction) {
  ...
  if (result.success) {
    embed.setFooter({ text: 'NEW: Use /verification admin og-sync to apply this role' });
  }

  await interaction.editReply({ embeds: [embed] });
  logger.log(`Admin ${interaction.user.tag} set OG role to ${role.name} (legacy command)`);
}
...
async function handleSetLimit(interaction) {
  ...
  if (result.success) {
    embed.setFooter({ text: 'NEW: Use /verification admin og-sync to apply the new limit' });
  }

  await interaction.editReply({ embeds: [embed] });
  logger.log(`Admin ${interaction.user.tag} set OG limit to ${count} (legacy command)`);
}
...
async function handleSync(interaction) {
  ...
  logger.log(`Admin ${interaction.user.tag} ran OG sync: +${result.added} -${result.removed} errors:${result.errors} (legacy command)`);
}
