...
const REQUIRED_SCHEMA = Object.freeze({
  ...
  wallets: ['discord_id', 'chain_family', 'chain_id', 'wallet_address', 'primary_wallet', 'is_favorite'],
  wallet_delegations: ['discord_id', 'guild_id', 'delegate_wallet_address', 'cold_wallet_address', 'status', 'expires_at'],
  wallet_verification_challenges: ['challenge_id', 'session_hash', 'discord_id', 'guild_id', 'chain_family', 'chain_id', 'wallet_address', 'message', 'expires_at', 'consumed_at', 'attempts'],
  ...
  token_role_rules: ['guild_id', 'chain_family', 'chain_id', 'token_mint', 'role_id', 'enabled', 'never_remove'],
  tenant_role_configs: ['guild_id', 'tiers_json', 'traits_json'],
  nft_tracked_collections: ['guild_id', 'chain_family', 'chain_id', 'collection_address', 'track_bid'],
  tracked_tokens: ['guild_id', 'chain_family', 'chain_id', 'alert_channel_ids'],
  tracked_wallets: ['guild_id', 'chain_family', 'chain_id', 'wallet_address', 'token_last_signature'],
  tracked_token_events: ['guild_id', 'chain_family', 'chain_id', 'wallet_address', 'token_mint', 'amount_raw', 'tx_signature'],
  nft_activity_events: ['chain_family', 'chain_id', 'collection_key', 'tx_signature'],
  evm_tracker_cursors: ['guild_id', 'chain_id', 'tracker_type', 'tracker_id', 'last_block', 'updated_at'],
  ...
  crypto_payment_receipts: ['guild_id', 'tx_signature', 'amount', 'token_symbol', 'sender_wallet', 'plan_key', 'billing_interval', 'status', 'verification_error', 'verified_at', 'created_at', 'custom_plan_json', 'quote_usd_amount', 'pricing_reason'],
  tenant_plan_contracts: ['tenant_id', 'base_plan_key', 'custom_plan_json', 'quoted_monthly_usd', 'billing_interval', 'assignment_source', 'pilot_plan_key', 'pilot_started_at', 'pilot_ends_at', 'pilot_status', 'pilot_assigned_by', 'pilot_note', 'updated_at'],
  ...
  webhook_event_inbox: ['inbox_id', 'event_type', 'source', 'payload_json', 'dedupe_key', 'status', 'attempts', 'next_attempt_at', 'last_error'],
...
function initDatabase() {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS tenant_welcome_settings (
      guild_id TEXT PRIMARY KEY,
      ...
      welcome_image_asset_id INTEGER DEFAULT NULL,
      dynamic_avatar_card INTEGER DEFAULT 0,
      dm_enabled INTEGER DEFAULT 0,
      dm_message_template TEXT DEFAULT NULL,
      auto_role_ids TEXT DEFAULT '[]',
      captcha_enabled INTEGER DEFAULT 0,
      captcha_role_id TEXT DEFAULT NULL,
      captcha_remove_role_id TEXT DEFAULT NULL,
      captcha_prompt_mode TEXT DEFAULT 'dm',
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
    )
  `);
  ...
  `);
  db.exec(`
    CREATE TABLE IF NOT EXISTS ai_assistant_knowledge_docs (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      guild_id TEXT NOT NULL,
      title TEXT NOT NULL,
      body TEXT NOT NULL,
  ...
  `);
  db.exec(`
    CREATE TABLE IF NOT EXISTS ai_assistant_channel_policies (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      guild_id TEXT NOT NULL,
      channel_id TEXT NOT NULL,
      mode TEXT NOT NULL DEFAULT 'mention',
      min_confidence INTEGER DEFAULT 35,
      passive_cooldown_seconds INTEGER DEFAULT 120,
      passive_max_per_hour INTEGER DEFAULT 6,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  ...
  ignoreDuplicateMigration(() => db.exec('ALTER TABLE ai_assistant_channel_policies ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP'));
