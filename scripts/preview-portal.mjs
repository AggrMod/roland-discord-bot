...
app.post('/api/admin/guildguard/incidents/:incidentId/threat-intelligence', (req, res) => res.status(201).json({ success: true, result: { incidentId: req.params.incidentId, entries: [{ domain: 'guildpiIot.example', status: 'pending', confidence: 50, report_count: 1, source_guild_count: 1 }] } }));
...
app.get('/api/admin/guildguard/summary', (_req, res) => res.json({ success: true, summary: { total: 12, statuses: { open: 2, reviewed: 7, confirmed: 2, false_positive: 1 }, averageRiskScore: 61, falsePositiveRate: 8.3, byDetector: [{ detector: 'link_protection', count: 6, averageScore: 64 }], actionStatuses: { applied: 8, failed: 0, skipped: 3 }, topChannels: [{ channelId: '100000000000000011', count: 5 }], openMemberReports: 1, recommendations: ['Links are the leading signal; keep official community domains in the trusted list.'], lastIncidentAt: '2026-08-04 08:12:00' } }));
app.get('/api/admin/guildguard/threat-intelligence', (_req, res) => res.json({ success: true, active: [{ domain: 'known-drainer.example', status: 'active', confidence: 90, report_count: 4, source_guild_count: 3 }], submissions: [{ domain: 'guildpiIot.example', status: 'pending', confidence: 50, report_count: 1, source_guild_count: 1 }] }));
app.get('/api/admin/guildguard/threat-intelligence/review-queue', (_req, res) => res.json({ success: true, pending: [{ domain: 'guildpiIot.example', status: 'pending', confidence: 50, report_count: 1, source_guild_count: 1 }] }));
app.post('/api/admin/guildguard/threat-intelligence/:domain/review', (req, res) => res.json({ success: true, entry: { domain: req.params.domain, status: req.body?.status || 'active' } }));
