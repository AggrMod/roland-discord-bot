...
class GameNightService {
  ...
  _author(embed, guildId) {
    try { const br = getBranding(guildId, 'battle'); embed.setAuthor({ name: br.brandName || 'Guild Pilot', ...(br.logo ? { iconURL: br.logo } : {}) }); } catch {}
  }
  ...
  buildLobbyEmbed(session, guildId) {
    ...
    applyEmbedBranding(e, { guildId, moduleKey: 'battle', defaultColor: '#f59e0b', defaultFooter: `Starts in ${session.gatherSecs}s · Host can react ${SKIP_EMOJI} to skip a game mid-session` });
    return e;
  }
  ...
  buildIntroEmbed({ gameIndex, total, gameName, gameDesc, guildId }) {
    const e = new EmbedBuilder()
      ...
      .setTimestamp();
    this._author(e, guildId);
    applyEmbedBranding(e, { guildId, moduleKey: 'battle', defaultColor: '#6366f1', defaultFooter: `Game Night · ${gameIndex}/${total}` });
    return e;
  }
  ...
  buildLeaderboardEmbed(session, guildId, afterGame, gamesLeft) {
    ...
    const e = new EmbedBuilder()
      ...
      .setTimestamp();
    this._author(e, guildId);
    applyEmbedBranding(e, { guildId, moduleKey: 'battle', defaultColor: '#6366f1', defaultFooter: 'GuildPilot · Game Night' });
    return e;
  }
  ...
  buildChampionEmbed(session, guildId, totalGames) {
    ...
    const e = new EmbedBuilder()
      ...
      .setTimestamp();
    this._author(e, guildId);
    applyEmbedBranding(e, { guildId, moduleKey: 'battle', defaultColor: '#f59e0b', defaultFooter: 'GuildPilot · Game Night Champion' });
    return e;
  }
  ...
  buildCancelledEmbed(reason, guildId) {
    ...
    this._author(e, guildId);
    applyEmbedBranding(e, { guildId, moduleKey: 'battle', defaultColor: '#64748b', defaultFooter: 'GuildPilot · Game Night' });
    return e;
  }
...
RUNNERS.diceduel = async (channel, guildId, players, playerNames, session) => {
  ...
  while (alive.size > 1) {
    ...
    if (isTie) {
      ...
      applyEmbedBranding(te, { guildId, moduleKey: 'battle', defaultColor: '#f59e0b', defaultFooter: `Game Night · Dice Duel Round ${round}` });
      await channel.send({ embeds: [te] });
    } else {
      const { EmbedBuilder: EB } = require('discord.js');
      const e = new EB().setTitle(`🎲 Round ${round}`).setDescription(lines).setTimestamp();
      applyEmbedBranding(e, { guildId, moduleKey: 'battle', defaultColor: '#6366f1', defaultFooter: `Game Night · Dice Duel Round ${round}` });
  ...
  if (winner) {
    ...
    applyEmbedBranding(we, { guildId, moduleKey: 'battle', defaultColor: '#4ade80', defaultFooter: 'Game Night · Dice Duel' });
...
RUNNERS.higherlower = async (channel, guildId, players, playerNames, session) => {
  ...
  while (alive.size > 1 && deck.length > 1) {
    ...
    const qEmbed = new EB()
      ...
      .setTimestamp();
    applyEmbedBranding(qEmbed, { guildId, moduleKey: 'battle', defaultColor: '#6366f1', defaultFooter: `Game Night · Higher or Lower · ${alive.size} left` });
    ...
    const rEmbed = new EB()
      ...
      .setTimestamp();
    applyEmbedBranding(rEmbed, { guildId, moduleKey: 'battle', defaultColor: actualElim.length > 0 ? '#ef4444' : '#4ade80', defaultFooter: `Game Night · Higher or Lower` });
  ...
  if (winner) {
    ...
    applyEmbedBranding(we, { guildId, moduleKey: 'battle', defaultColor: '#4ade80', defaultFooter: 'Game Night · Higher or Lower' });
    await channel.send({ embeds: [we] });
  }
...
RUNNERS.reactionrace = async (channel, guildId, players, playerNames, session) => {
  ...
  while (alive.size > 1) {
    ...
    applyEmbedBranding(readyE, { guildId, moduleKey: 'battle', defaultColor: '#6366f1', defaultFooter: `Game Night · Reaction Race · ${alive.size} left` });
    ...
    applyEmbedBranding(goE, { guildId, moduleKey: 'battle', defaultColor: '#f59e0b', defaultFooter: `Game Night · Reaction Race Round ${round}` });
    ...
    applyEmbedBranding(resE, { guildId, moduleKey: 'battle', defaultColor: '#ef4444', defaultFooter: `Game Night · Reaction Race` });
  ...
  if (winner) {
    ...
    applyEmbedBranding(we, { guildId, moduleKey: 'battle', defaultColor: '#4ade80', defaultFooter: 'Game Night · Reaction Race' });
    await channel.send({ embeds: [we] });
  }
...
RUNNERS.numberguess = async (channel, guildId, players, playerNames, session) => {
  ...
  for (let round = 1; round <= ROUNDS; round++) {
    ...
    applyEmbedBranding(qE, { guildId, moduleKey: 'battle', defaultColor: '#6366f1', defaultFooter: `Game Night · Number Guess Round ${round}` });
    ...
    applyEmbedBranding(rE, { guildId, moduleKey: 'battle', defaultColor: '#4ade80', defaultFooter: `Game Night · Number Guess Round ${round}` });
  ...
  applyEmbedBranding(wE, { guildId, moduleKey: 'battle', defaultColor: '#f59e0b', defaultFooter: 'Game Night · Number Guess' });
  await channel.send({ embeds: [wE] });
...
RUNNERS.slots = async (channel, guildId, players, playerNames, session) => {
  ...
  applyEmbedBranding(e, { guildId, moduleKey: 'battle', defaultColor: '#f59e0b', defaultFooter: 'Game Night · Slots' });
  await channel.send({ embeds: [e] });
...
RUNNERS.trivia = async (channel, guildId, players, playerNames, session) => {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    applyEmbedBranding(qE, { guildId, moduleKey: 'battle', defaultColor: '#6366f1', defaultFooter: `Game Night · Trivia Q${i+1}` });
    ...
    applyEmbedBranding(aE, { guildId, moduleKey: 'battle', defaultColor: winners.length > 0 ? '#4ade80' : '#ef4444', defaultFooter: `Game Night · Trivia Q${i+1}` });
  ...
  applyEmbedBranding(wE, { guildId, moduleKey: 'battle', defaultColor: '#f59e0b', defaultFooter: 'Game Night · Trivia' });
  await channel.send({ embeds: [wE] });
  return sorted.map(([id]) => id);
};
...
RUNNERS.wordscramble = async (channel, guildId, players, playerNames, session) => {
  ...
  for (let round = 1; round <= pool.length; round++) {
    ...
    applyEmbedBranding(qE, { guildId, moduleKey: 'battle', defaultColor: '#6366f1', defaultFooter: `Game Night · Word Scramble Round ${round}` });
    await channel.send({ embeds: [qE] });
    ...
    applyEmbedBranding(rE, { guildId, moduleKey: 'battle', defaultColor: winnerId ? '#4ade80' : '#ef4444', defaultFooter: `Game Night · Word Scramble` });
    await channel.send({ embeds: [rE] });
    await sleep(2000);
  }
  ...
  applyEmbedBranding(wE, { guildId, moduleKey: 'battle', defaultColor: '#f59e0b', defaultFooter: 'Game Night · Word Scramble' });
  await channel.send({ embeds: [wE] });
  return sorted.map(([id]) => id);
};
...
RUNNERS.rps = async (channel, guildId, players, playerNames, session) => {
  ...
  while (alive.size > 1) {
    ...
    if (bye) { const bE = new EB().setTitle(`🪨 Round ${round} — Bye`).setDescription(`**${playerNames.get(bye) || bye}** gets a free pass this round! 🍀`).setTimestamp(); applyEmbedBranding(bE,{guildId,moduleKey:'battle',defaultColor:'#4ade80',defaultFooter:`Game Night · RPS Round ${round}`}); await channel.send({embeds:[bE]}); }
    ...
    for (const [pA, pB] of matchups) {
      ...
      applyEmbedBranding(mE,{guildId,moduleKey:'battle',defaultColor:'#e74c3c',defaultFooter:`Game Night · RPS Round ${round}`});
      ...
      applyEmbedBranding(resE,{guildId,moduleKey:'battle',defaultColor:loser?'#ef4444':'#6366f1',defaultFooter:`Game Night · RPS`});
      await channel.send({embeds:[resE]});
  ...
  if (winner) {
    ...
    applyEmbedBranding(wE,{guildId,moduleKey:'battle',defaultColor:'#4ade80',defaultFooter:'Game Night · RPS'});
    await channel.send({embeds:[wE]});
  }
  return winner ? [winner, ...eliminated] : [...eliminated];
};
...
RUNNERS.blackjack = async (channel, guildId, players, playerNames, session) => {
  ...
  const tableE = new EB().setTitle('🎴 Blackjack — Cards Dealt!')
    ...
    .setTimestamp();
  applyEmbedBranding(tableE, {guildId, moduleKey:'battle', defaultColor:'#6366f1', defaultFooter:'Game Night · Blackjack'});
  ...
  for (const id of players) {
    ...
    const tE = new EB().setTitle(`🎴 ${playerNames.get(id)||id}'s turn!`)
      ...
      .setTimestamp();
    applyEmbedBranding(tE, {guildId, moduleKey:'battle', defaultColor:'#6366f1', defaultFooter:'Game Night · Blackjack'});
    ...
    while (!standing && handVal(hand) < 21) {
      ...
      applyEmbedBranding(updE,{guildId,moduleKey:'battle',defaultColor:val>21?'#ef4444':'#6366f1',defaultFooter:'Game Night · Blackjack'});
  ...
  applyEmbedBranding(resE,{guildId,moduleKey:'battle',defaultColor:'#f59e0b',defaultFooter:'Game Night · Blackjack'});
  await channel.send({embeds:[resE]});
  return results.map(r => r.id);
};
